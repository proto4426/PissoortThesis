...
#corr structures available in nlme (using corr structures beyond the strictly additive case amounts to using a GEE approach to fitting).
summary(gam1$gam) # The smoother explains 20% of the variance of the data. 3 df 

# look at autocorrelation in residuals:
acfpacf_plot(resid(gam1$lme, type = "normalized"))
#  Hard to choose for AR or MA terms visually here...  whould do further examinations (!)
...
anova(gam1$lme, gam2$lme, gam3$lme, gam4$lme, gam5$lme, gam6$lme)
...
sims <- simulate(gam1, nsim = 10000, newdata = newd) # See Fun.R (??)
...
g_deriv_pointw <- ggplot(newd, aes(x = Year, y = derivative)) +
  ...
  labs(title = "Estimated first derivative of the fitted spline with .95 pointw interval",
       y = "First derivative", x = "Year")+
...
## fit an additive model with seasonal and trend smooth and an AR(2) process for the residuals; the code predicts from the model at 200 locations over the entire time series and generates a pointwise, approximate 95% confidence interval on the trend spline.
# Reuse aboe code (gam3, with MA(1) fitted on the errors)
...
plot(p2 ~ Year, data = pdat, type = "n", ylab = ylab, ylim = ylim)
...
gam3.0 <- gam(Max ~ s(Year, k = 20), data = max_years$df,
              method = "REML") # Remove correlation structure
...
grid <- 200 # Defines grid of values for which we will generate predictions.
...
#  ===========================================================================
## Simultaneous interval for derivatives of smooths (revisited, 2017) 
...
g_deriv_simult <- 
  ...
  labs(title = "Estimated first derivative of the fitted spline with .95 simult interval",
       y = "First derivative", x = "Year") + theme_piss()
g_deriv_simult
...
# Notice large uncertainty... 


## Coverage properties  (Reuse inCI function defined above)
